...
import { getRepositories } from '../../../helpers/GitHubApi';
...
const Repositories = ({query}) => { 
    ...
    useEffect(() => {
        ...
        async function fetch() {
            const results = await getRepositories(query, access);
    ...
    return (
       <Row>
            {repositories === null ? <Col md="12" lg="12" sm="12"><Alert color="warning" className="m-3">Carregando Resultados</Alert></Col> : 
                repositories && Array.isArray(repositories) && repositories.length > 0 ? repositories.map((repositorie) => (
